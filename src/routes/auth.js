...
router.get('/auth/gmail/connect', async (req, res) => {
  if (!gmailConnectAuthorized(req)) {
    return res.status(401).send('Unauthorized — pass ?secret=PRIMARY_GMAIL_CONNECT_SECRET');
  }
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      return res.status(400).send(
        'No refresh_token returned. Revoke app access at https://myaccount.google.com/permissions then retry /auth/gmail/connect.'
      );
    }
