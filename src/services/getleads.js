...
/**
 * Look up a contact by work email and return cellphone/LinkedIn/website when present.
 */
async function findPhoneByEmail(email) {
  const workEmail = String(email || '').trim().toLowerCase();
  if (!workEmail || !workEmail.includes('@')) {
    return { phone: null, linkedinUrl: null, website: null, source: 'getleads', skipped: 'no_email' };
  }
  if (!isConfigured()) {
    return { phone: null, linkedinUrl: null, website: null, source: 'getleads', skipped: 'not_configured' };
  }
  ...
  if (!contact) {
    return { phone: null, linkedinUrl: null, website: null, source: 'getleads', skipped: 'not_found' };
  }
  ...
  const website = contact.company_website || contact.website || null;
  return {
    phone,
