/**
 * Primary-domain Gmail sender (joshua@salesglidergrowth).
 * OAuth client: GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET (separate from calendar Google OAuth).
 */
...
function expectedFromEmail() {
  return String(process.env.PRIMARY_GMAIL_FROM || 'joshua@salesglidergrowth.com').trim().toLowerCase();
}
...
async function getValidAccessToken() {
  ...
  if (!account) {
    throw new Error(
      `Primary Gmail not connected — visit /auth/gmail/connect (expect ${expectedFromEmail()})`
    );
  }
